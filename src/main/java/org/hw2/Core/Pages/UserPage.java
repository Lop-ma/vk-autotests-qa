...
public class UserPage extends LoadablePage {
    ...
    public UserPage checkPage() {
        $(ACTIVE_MENU_NAME)
                .shouldBe(enabled.because(MENU_NAME_ON_USER_PAGE));
        $(PROFILE_USER_NAME)
                .shouldBe(visible.because(PROFILE_USER_NAME_ON_USER_PAGE));
        $(USER_AVATAR)
                .shouldBe(enabled.because(USER_AVATAR_ON_USER_PAGE));
        $(ABOUT_USER)
                .shouldBe(enabled.because(ABOUT_USER_ON_USER_PAGE));
        $(FAVORITE_PHOTOS)
                .shouldBe(enabled.because(FAVORITE_PHOTOS_ON_USER_PAGE));
        return this;
    }

    public UserPage deleteLastPost() {
        $(FIRST_POST_MENU)
                .shouldBe(enabled.because(POST_MENU_FOR_DELETE_POST))
                .hover();
        $(DELETE_POST_BUTTON)
                .shouldBe(enabled.because(DELETE_BUTTON_FOR_DELETE_POST))
                .click();
        $(CONFIRM_DELETE)
                .shouldBe(enabled.because(BUTTON_CONFIRM_FOR_DELETE_POST))
                .click();
        return this;
    }
    ...
    public UserPage likeLastPost() {
        $(LIKE_BUTTON)
                .shouldBe(enabled.because(LIKE_BUTTON_FOR_PUT_LIKE))
                .click();
        return this;
    }
    ...
    public boolean shouldHaveLikeOnLastPost() {
        $(ACTIVE_LIKE)
                .shouldBe(enabled.because(ACTIVE_LIKE_FOR_CHECK_PUT_LIKE));
        return true;
    }
}
